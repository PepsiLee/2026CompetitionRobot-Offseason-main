...
/** Coordinates the fixed intake, gravity-fed shooter, and stationary chassis aiming. */
public final class SuperStructure extends SubsystemBase {
  ...
  private static final double AIM_MAX_ANGULAR_SPEED_RADIANS_PER_SECOND =
  ...
  private final Debouncer aimReadyDebouncer =
      new Debouncer(AIM_STABLE_SECONDS, Debouncer.DebounceType.kRising);
  ...
  public SuperStructure(Drive drive, RobotState robotState, Intake intake, Shooter shooter) {
    this.drive = drive;
  ...
  private SystemState determineState() {
    ...
    if (systemState == SystemState.SHOOTING) {
      if (Math.abs(drive.getHeadingErrorRadians(lockedShotHeading))
          > AIM_EXIT_TOLERANCE_RADIANS) {
        aimReadyDebouncer.calculate(false);
    ...
    boolean rawAimReady =
        drive.isAtHeading(desiredAimHeading, AIM_ENTER_TOLERANCE_RADIANS)
            && Math.abs(drive.getAngularVelocityRadiansPerSecond())
    ...
    lockedShotDistanceMeters =
        robotState.getPose().getTranslation().getDistance(
  ...
  private void applyState() {
    ...
    switch (systemState) {
      ...
      case SHOOTING -> {
        shooter.setWantedState(Shooter.WantedState.RUN);

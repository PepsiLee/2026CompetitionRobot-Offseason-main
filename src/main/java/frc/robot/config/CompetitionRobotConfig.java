 ...
 *
 * <p>Hardware values are synchronized with the robot's Phoenix Tuner X swerve project.
 */
public final class CompetitionRobotConfig implements RobotConfiguration {
  private static final String CAN_BUS = RobotBase.isSimulation() ? "*" : "";
  ...
  private static final double WHEELBASE_METERS = Units.inchesToMeters(12.5);
  private static final double TRACK_WIDTH_METERS = Units.inchesToMeters(12.5);
  ...
  private static final ModuleHardware FRONT_LEFT =
      new ModuleHardware(1, 2, 11, 0.094482421875, false);
  private static final ModuleHardware FRONT_RIGHT =
      new ModuleHardware(3, 4, 12, 0.003173828125, true);
  private static final ModuleHardware BACK_LEFT =
      new ModuleHardware(5, 6, 13, -0.36669921875, false);
  private static final ModuleHardware BACK_RIGHT =
      new ModuleHardware(7, 8, 14, 0.171142578125, true);
  ...
  public CompetitionRobotConfig() {
    ...
    // CAN 9 runs at +7 V whenever teleop is enabled. CAN 10 runs at +3 V while Circle is held
    // (and while an autonomous intake request is active).
    intakeConfiguration =
        new IntakeConfiguration(CAN_BUS, 9, 10, 7.0, 3.0, 40.0, 80.0);

    // All three shooter motors are independent VoltageOut devices; none is configured as a
    ...
    // Provisional rear-facing transform. Replace with a measured robot-to-camera transform.
    visionConfiguration =
